...
def get_overview_kpis(start=None, end=None):
        ...
        try:
            with conn.cursor() as cur:
                ...
                for key, (tbl, cnd) in tables.items():
                    try:
                        cur.execute(f"SELECT COUNT(*) as v FROM {tbl} WHERE {cnd}")
                        row = cur.fetchone()
...
def get_hourly_activity(start=None, end=None):
    if not start and not end:
                ...
                for label, tbl in parts:
                    try:
                        cur.execute(
                            ...
                            f"COUNT(*) as cnt FROM {tbl} WHERE {cond} GROUP BY hour ORDER BY hour"
                        )
                        rows = cur.fetchall()
                        if rows:
...
def get_top_active_players(start=None, end=None, limit=10):
    if not start and not end:
        start, end = _default_dates()
    cond = f"DATE(timestamp) BETWEEN '{start}' AND '{end}'"
    sql = f"""
        SELECT player_name, COUNT(*) as events FROM (
            SELECT player_name, timestamp FROM attack_entity_events WHERE {cond}
            UNION ALL SELECT player_name, timestamp FROM block_break_events WHERE {cond}
            UNION ALL SELECT player_name, timestamp FROM block_place_events WHERE {cond}
            UNION ALL SELECT player_name, timestamp FROM item_drop_events WHERE {cond}
            UNION ALL SELECT player_name, timestamp FROM item_pickup_events WHERE {cond}
            UNION ALL SELECT player_name, timestamp FROM player_command_events WHERE {cond}
        ) combined
...
def get_login_sessions(players=None, start=None, end=None):
...
def get_session_stats(players=None, start=None, end=None):
    pf = _player_f("p.username", players)
    df = _date_f("pl.login_time", start, end)
    return query_df(f"""
        SELECT p.username as player,
...
def get_logins_per_day(players=None, start=None, end=None):
    pf = _player_f("p.username", players)
    df = _date_f("pl.login_time", start, end)
    return query_df(f"""
...
def get_attack_events(players=None, start=None, end=None, limit=1000):
...
def get_damage_stats(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT player_name,
...
def get_entity_hit_counts(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_time_to_kill(players=None, start=None, end=None, limit=1000):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT player_name, entity_type,
...
def get_ttk_by_entity(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT entity_type,
...
def get_player_kills(players=None, start=None, end=None, limit=1000):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_kills_per_player(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_deaths(players=None, start=None, end=None, limit=1000):
    pf = _player_f("p.username", players)
...
def get_death_causes(players=None, start=None, end=None):
    pf = _player_f("p.username", players)
    df = _date_f("d.timestamp", start, end)
    return query_df(f"""
...
def get_deaths_per_day(players=None, start=None, end=None):
    pf = _player_f("p.username", players)
    df = _date_f("d.timestamp", start, end)
    return query_df(f"""
...
def get_deadliest_players(players=None, start=None, end=None):
    pf = _player_f("p.username", players)
    df = _date_f("d.timestamp", start, end)
    return query_df(f"""
...
def get_block_breaks(players=None, start=None, end=None, limit=500):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_top_blocks_broken(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_block_break_by_player(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_block_places(players=None, start=None, end=None, limit=500):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_top_blocks_placed(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT block_type, COUNT(*) as count
...
def get_block_interact(players=None, start=None, end=None, limit=500):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT player_name, block_type, dimension, block_x, block_y, block_z,
...
def get_fluid_places(players=None, start=None, end=None, limit=500):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_entity_interact(players=None, start=None, end=None, limit=500):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_item_drops(players=None, start=None, end=None, limit=500):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_item_pickups(players=None, start=None, end=None, limit=500):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT player_name, item_type, item_count, dimension,
...
def get_top_items_dropped(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT item_type, SUM(item_count) as total, COUNT(*) as events
        FROM item_drop_events WHERE 1=1 {pf} {df}
        GROUP BY item_type ORDER BY total DESC LIMIT 25
    """)


def get_top_items_picked(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT item_type, SUM(item_count) as total, COUNT(*) as events
        FROM item_pickup_events WHERE 1=1 {pf} {df}
        GROUP BY item_type ORDER BY total DESC LIMIT 25
    """)


def get_crafting(players=None, start=None, end=None, limit=500):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_top_crafted(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT crafted_item, COUNT(*) as count
        FROM crafting_event WHERE 1=1 {pf} {df}
        GROUP BY crafted_item ORDER BY count DESC LIMIT 25
    """)


def get_item_consume(players=None, start=None, end=None, limit=500):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_top_consumed(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT item_type, COUNT(*) as uses,
               ROUND(AVG(player_hp_after - player_hp_before), 2) as avg_hp_gain
        FROM item_consume_events WHERE 1=1 {pf} {df}
...
def get_chat(players=None, start=None, end=None, limit=500, search=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
...
def get_chat_per_player(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_commands(players=None, start=None, end=None, limit=500, search=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
...
def get_top_commands(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
        SELECT SUBSTRING_INDEX(LTRIM(command), ' ', 1) as cmd, COUNT(*) as uses
        FROM player_command_events WHERE 1=1 {pf} {df}
...
def get_advancements(players=None, start=None, end=None, limit=500):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_advancement_stats(players=None, start=None, end=None):
    pf = _player_f("player_name", players)
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_first_to_advance(start=None, end=None):
    df = _date_f("timestamp", start, end)
    return query_df(f"""
...
def get_table_data(table_name, players=None, start=None, end=None, limit=2000):
    ...
    return query_df(f"SELECT * FROM {table_name} WHERE 1=1 {pf} {df} ORDER BY {ts_col} DESC LIMIT {limit}")
...
def get_player_timeline(player, start=None, end=None, limit=2000):
    ...
    queries = [
        ("Attack", f"""
            ...
            FROM attack_entity_events WHERE player_name='{p}' {df_ts}
        """),
        ("Kill", f"""
            SELECT timestamp as ts, 'Kill' as type,
                CONCAT('Killed ', entity_type, '  [', COALESCE(main_hand_item,'?'), ']') as details,
                dimension, player_x as x, player_y as y, player_z as z
            FROM player_kill_events WHERE player_name='{p}' {df_ts}
        """),
        ("Death", f"""
...
def get_pvp_incidents(start=None, end=None, victim=None, killer=None, limit=1000):
    ...
    base = f"""
        FROM player_deaths d
        JOIN players p ON d.player_uuid = p.uuid
        WHERE d.killer_name IS NOT NULL {dts} {vf} {kf}
        ORDER BY d.timestamp DESC LIMIT {limit}
    """
...
def get_item_transfers(start=None, end=None, time_window_sec=300, max_distance=30, limit=500):
...
def get_events_near(cx, cz, radius, start=None, end=None, limit=500):
    ...
    FROM (
        SELECT timestamp as ts, 'Attack' as event_type, player_name as player,
            ...
            dimension, player_x as x, player_y as y, player_z as z
        FROM attack_entity_events
        WHERE ABS(player_x - {cx}) <= {r} AND ABS(player_z - {cz}) <= {r} {df_ts}
        ...
        WHERE d.x IS NOT NULL AND ABS(d.x - {cx}) <= {r} AND ABS(d.z - {cz}) <= {r} {df_d}
        ...
        FROM block_break_events
        WHERE ABS(block_x - {cx}) <= {r} AND ABS(block_z - {cz}) <= {r} {df_ts}
...
def get_player_proximity(player_a, player_b, start=None, end=None, distance=50, limit=500):
    ...
    FROM (
        ...
        FROM block_break_events WHERE player_name='{pa}' {df_a}
        UNION ALL
        SELECT timestamp, player_x, player_y, player_z
        FROM block_place_events WHERE player_name='{pa}' {_date_f('timestamp', start, end)}
        UNION ALL
        SELECT timestamp, player_x, player_y, player_z
        FROM attack_entity_events WHERE player_name='{pa}' {_date_f('timestamp', start, end)}
    ) a
    JOIN (
        SELECT timestamp as ts2, player_x as bx, player_y as by, player_z as bz
        FROM block_break_events WHERE player_name='{pb}' {_date_f('timestamp', start, end)}
        UNION ALL
        SELECT timestamp, player_x, player_y, player_z
        FROM block_place_events WHERE player_name='{pb}' {_date_f('timestamp', start, end)}
        UNION ALL
        SELECT timestamp, player_x, player_y, player_z
        FROM attack_entity_events WHERE player_name='{pb}' {_date_f('timestamp', start, end)}
...
def get_map_data(event_type, players=None, start=None, end=None, limit=5000):
    ...
    queries = {
        "deaths": (
            f"""SELECT p.username as player,
                   ...
                   DATE_FORMAT(d.timestamp,'%Y-%m-%d %H:%i:%s') as timestamp
            FROM player_deaths d JOIN players p ON d.player_uuid=p.uuid
            WHERE d.x IS NOT NULL {df_d} {pf_p}""",
        ...
        "pvp": (
            f"""SELECT p.username as player,
                   ...
                   DATE_FORMAT(d.timestamp,'%Y-%m-%d %H:%i:%s') as timestamp
            FROM player_deaths d JOIN players p ON d.player_uuid=p.uuid
            WHERE d.killer_name IS NOT NULL {df_d} {pf_p}""",
        ...
        "attacks": (
            ...
            f"""SELECT player_name as player,
                   ...
                   CONCAT('Hit ', entity_type, ' for ', ROUND(actual_damage,1), ' dmg') as details,
                   NULL as dimension, DATE_FORMAT(timestamp,'%Y-%m-%d %H:%i:%s') as timestamp
            FROM attack_entity_events WHERE 1=1 {df_ts} {pf_n}""",
        ),
        "block_breaks": (
            f"""SELECT player_name as player,
                   ...
                   dimension, DATE_FORMAT(timestamp,'%Y-%m-%d %H:%i:%s') as timestamp
            FROM block_break_events
            WHERE block_x IS NOT NULL {df_ts} {pf_n}""",
            None,
        ),
        "block_places": (
            f"""SELECT player_name as player,
        ...
        "drops": (
            ...
            f"""SELECT player_name as player,
                   ...
                   CONCAT('Dropped ', item_count, 'x ', item_type) as details,
                   NULL as dimension, DATE_FORMAT(timestamp,'%Y-%m-%d %H:%i:%s') as timestamp
            FROM item_drop_events WHERE 1=1 {df_ts} {pf_n}""",
        ),
        "pickups": (
            f"""SELECT player_name as player,
                   ROUND(player_x,1) as map_x, ROUND(player_z,1) as map_z, ROUND(player_y,1) as y,
            ...
            f"""SELECT player_name as player,
                   ...
                   CONCAT('Picked up ', item_count, 'x ', item_type) as details,
                   NULL as dimension, DATE_FORMAT(timestamp,'%Y-%m-%d %H:%i:%s') as timestamp
            FROM item_pickup_events WHERE 1=1 {df_ts} {pf_n}""",
        ),
        "crafts": (
            f"""SELECT player_name as player,
                   ROUND(player_x,1) as map_x, ROUND(player_z,1) as map_z, ROUND(player_y,1) as y,
            ...
            f"""SELECT player_name as player,
                   ...
                   CONCAT('Crafted ', COALESCE(crafted_item,'?')) as details,
                   NULL as dimension, DATE_FORMAT(timestamp,'%Y-%m-%d %H:%i:%s') as timestamp
            FROM crafting_event WHERE 1=1 {df_ts} {pf_n}""",
        ),
    }
