...
public class TradeMenu extends AbstractContainerMenu {
    ...
    private void layoutSlots(Inventory playerInventory) {
        // GUI is 276x240 pixels (wider for two-column layout)
        ...
        for (int row = 0; row < 6; row++) {
            for (int col = 0; col < 4; col++) {
                this.addSlot(new Slot(playerOffer, col + row * 4, playerOfferX + col * 18, playerOfferY + row * 18));
            }
        }
    ...
    public void removed(Player player) {
        ...
        // Return items to player when menu is closed
