...
import com.THproject.tharidia_things.client.model.ChimneyItemModel;
...
/**
 * GeckoLib renderer for ChimneyItem.
 * Shows only the stage_4 bone in the GUI.
 */
public class ChimneyItemRenderer extends GeoItemRenderer<ChimneyItem> {

    private static final float SCALE = 0.6f;  // 4x original

    public ChimneyItemRenderer() {
        super(new ChimneyItemModel());
    }

    @Override
    public void preRender(PoseStack poseStack, ChimneyItem animatable, BakedGeoModel model,
                          @Nullable MultiBufferSource bufferSource, @Nullable VertexConsumer buffer,
                          boolean isReRender, float partialTick, int packedLight, int packedOverlay,
                          int colour) {
        super.preRender(poseStack, animatable, model, bufferSource, buffer, isReRender, partialTick, packedLight, packedOverlay, colour);

        // Hide all bones except stage_4 (chimney)
        hideAllExcept(model, "stage_4");
    }
    ...
    public void scaleModelForRender(float widthScale, float heightScale, PoseStack poseStack,
                                    ChimneyItem animatable, BakedGeoModel model, boolean isReRender,
