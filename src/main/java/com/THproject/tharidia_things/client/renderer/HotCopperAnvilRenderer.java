...
/**
 * Renders the hot copper on the anvil with temperature gradient,
...
public class HotCopperAnvilRenderer implements BlockEntityRenderer<HotCopperAnvilEntity> {
    ...
    public HotCopperAnvilRenderer(BlockEntityRendererProvider.Context context) {
        ...
        this.hotCopperModels = new BakedModel[5];
    }
    ...
    private float getCoolingProgress(HotCopperAnvilEntity entity) {
    ...
    private float[] getTemperatureColor(float cooling) {
        // Copper has a reddish-orange glow
        if (cooling < 0.25f) {
            float t = cooling / 0.25f;
            return new float[]{1.0f, 0.9f - t * 0.15f, 0.7f - t * 0.3f};
        } else if (cooling < 0.5f) {
            float t = (cooling - 0.25f) / 0.25f;
            return new float[]{1.0f, 0.75f - t * 0.3f, 0.4f - t * 0.2f};
        } else if (cooling < 0.75f) {
            float t = (cooling - 0.5f) / 0.25f;
            return new float[]{1.0f - t * 0.25f, 0.45f - t * 0.25f, 0.2f - t * 0.1f};
        } else {
            float t = (cooling - 0.75f) / 0.25f;
            return new float[]{0.75f - t * 0.3f, 0.2f + t * 0.1f, 0.1f + t * 0.2f};
        }
    }

    private void renderHotspot(HotCopperAnvilEntity entity, PoseStack poseStack,
        ...
        for (int ring = 0; ring < 5; ring++) {
            ...
            // Copper-orange glow
            renderCircle(poseStack, buffer, radius, 1.0f, 0.55f, 0.15f, alpha);
        }

        if (pulse > 0.6f) {
            float centerAlpha = (pulse - 0.6f) / 0.4f * 0.9f;
            renderCircle(poseStack, buffer, hotspotSize * 0.08f, 1.0f, 0.9f, 0.5f, centerAlpha);
        }

        poseStack.popPose();
    }

    private void renderProgressDiamonds(HotCopperAnvilEntity entity, PoseStack poseStack,
                                         MultiBufferSource buffer) {
        poseStack.pushPose();
