...
import com.THproject.tharidia_things.client.model.CrucibleItemModel;
...
/**
 * GeckoLib renderer for CrucibleItem.
 * Shows only the stage_2 bone in the GUI.
 */
public class CrucibleItemRenderer extends GeoItemRenderer<CrucibleItem> {
    ...
    public CrucibleItemRenderer() {
        super(new CrucibleItemModel());
    }

    @Override
    public void preRender(PoseStack poseStack, CrucibleItem animatable, BakedGeoModel model,
                          @Nullable MultiBufferSource bufferSource, @Nullable VertexConsumer buffer,
                          boolean isReRender, float partialTick, int packedLight, int packedOverlay,
                          int colour) {
        super.preRender(poseStack, animatable, model, bufferSource, buffer, isReRender, partialTick, packedLight, packedOverlay, colour);

        // Hide all bones except stage_2 (crucible)
        hideAllExcept(model, "stage_2");
    }
    ...
    public void scaleModelForRender(float widthScale, float heightScale, PoseStack poseStack,
                                    CrucibleItem animatable, BakedGeoModel model, boolean isReRender,
                                    float partialTick, int packedLight, int packedOverlay) {
        // Position model
        poseStack.translate(-0.8, -0.7, 0.0);
